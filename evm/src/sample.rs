    ...
    C::Hasher: AlgebraicHasher<F>,
{
    ...
    let (all_circuits, block_proof) = step3()?;
...
pub fn step1<F, C, const D: usize>() -> Result<()>
where
    F: RichField + Extendable<D>,
    C: GenericConfig<D, F = F> + 'static,
    C::Hasher: AlgebraicHasher<F>,
{
    let (
        ...
        _receipts_trie,
        inputs_first,
        _expected_state_trie_after,
        _code_hash,
        _code,
        _sender_balance_after,
        _gas_used,
        _to_account_after,
        _to_account_second_before,
        _to,
        _beneficiary_nibbles,
        _sender_nibbles,
        _to_nibbles,
        _to_second_nibbles,
    ) = generate_values!();
    ...
    // write to files
    std::fs::write("root_proof_first.json", root_proof_first_json).unwrap();
    std::fs::write("first_public_values.json", first_public_values_json).unwrap();

    Ok(())
}

pub fn step2<F, C, const D: usize>() -> Result<()>
where
    F: RichField + Extendable<D>,
    C: GenericConfig<D, F = F> + 'static,
    C::Hasher: AlgebraicHasher<F>,
{
    ...
    // read from files
    let root_proof_first_json = std::fs::read_to_string("root_proof_first.json").unwrap();
    ...
    // serialize
    let root_proof_json = serde_json::to_string(&root_proof).unwrap();
    let public_values_json = serde_json::to_string(&public_values).unwrap();

    // write to files
    std::fs::write("root_proof.json", root_proof_json).unwrap();
    std::fs::write("public_values.json", public_values_json).unwrap();

    Ok(())
}

pub fn step3<F, C, const D: usize>() -> Result<(
    AllRecursiveCircuits<F, C, D>,
    ProofWithPublicInputs<F, C, D>,
)>
where
...
mod tests {
    ...
    #[test]
    fn test_get_sample_circuits_and_proof_step1() {
        step1::<F, C, D>();
    }

    #[test]
    fn test_get_sample_circuits_and_proof_step2() {
        step2::<F, C, D>();
    }

    #[test]
    fn test_get_sample_circuits_and_proof_step3() {
