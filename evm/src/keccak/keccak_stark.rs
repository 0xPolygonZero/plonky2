...
use crate::keccak::columns::{reg_a, reg_a_prime, reg_a_prime_prime, reg_a_prime_prime_0_0_bit, reg_a_prime_prime_prime, reg_b, reg_c, reg_c_prime, reg_input_limb, reg_output_limb, reg_step, NUM_COLUMNS, REG_FILTER, reg_preimage};
...
impl<F: RichField + Extendable<D>, const D: usize> KeccakStark<F, D> {
    ...
    fn generate_trace_rows_for_perm(&self, input: [u64; NUM_INPUTS]) -> [[F; NUM_COLUMNS]; NUM_ROUNDS] {
        ...
        // log::info!("RESULT {}", rows[NUM_ROUNDS - 1][reg_a_prime_prime_prime(0, 0)]);
...
impl<F: RichField + Extendable<D>, const D: usize> Stark<F, D> for KeccakStark<F, D> {
    ...
    fn eval_ext_circuit(
        ...
        for x in 0..5 {
            for y in 0..5 {
                let preimage = reg_preimage(x, y);
                let diff = builder.sub_extension(vars.local_values[preimage], vars.next_values[preimage]);
