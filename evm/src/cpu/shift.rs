...
pub(crate) fn eval_packed<P: PackedField>(
    ...
    yield_constr.constraint(is_shift * high_limbs_are_zero * two_exp.addr_context); // read from kernel memory
    yield_constr
        .constraint(is_shift * high_limbs_are_zero * (two_exp.addr_segment - shift_table_segment));
    yield_constr.constraint(
        is_shift * high_limbs_are_zero * (two_exp.addr_virtual - displacement.value[0]),
    );
