...
%macro bn_mload_wnaf_a
    // stack: i
    %mload_current(@SEGMENT_KERNEL_BN_WNAF_A)
%endmacro

%macro bn_mload_wnaf_b
    // stack: i
    %mload_current(@SEGMENT_KERNEL_BN_WNAF_B)
%endmacro

%macro bn_mload_point_a
    // stack: w
    DUP1
    %mload_current(@SEGMENT_KERNEL_BN_TABLE_Q)
    //stack: Gy, w
    SWAP1 %decrement %mload_current(@SEGMENT_KERNEL_BN_TABLE_Q)
    //stack: Gx, Gy
%endmacro
...
%macro bn_mload_point_b
    ...
    DUP1
    %mload_current(@SEGMENT_KERNEL_BN_TABLE_Q)
    PUSH @BN_BNEG_LOC %mload_current(@SEGMENT_KERNEL_BN_TABLE_Q)
