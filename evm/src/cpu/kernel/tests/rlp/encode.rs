...
fn test_encode_rlp_scalar_medium() -> Result<()> {
    ...
    interpreter.run()?;
    let expected_stack = vec![U256::from(Segment::RlpRaw as usize + 6)]; // pos' = pos + rlp_len = 2 + 4
...
fn test_encode_rlp_160() -> Result<()> {
    ...
    interpreter.run()?;
    let expected_stack = vec![U256::from(Segment::RlpRaw as usize + 1 + 20)]; // pos'
    #[rustfmt::skip]
...
fn test_encode_rlp_256() -> Result<()> {
    ...
    interpreter.run()?;
    let expected_stack = vec![U256::from(Segment::RlpRaw as usize + 1 + 32)]; // pos'
    #[rustfmt::skip]
