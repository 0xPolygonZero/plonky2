...
fn test_erc20() -> anyhow::Result<()> {
    ...
    let giver_bits = giver_state_key.into();
    let token_bits = token_state_key.into();

    let mut state_smt_before = Smt::empty();
    state_smt_before
        .insert(sender_bits, sender_account().into())
        .unwrap();
    state_smt_before
        .insert(giver_bits, giver_account().into())
        .unwrap();
    state_smt_before
    ...
    let expected_state_trie_after = {
        let mut state_smt_after = Smt::empty();
        ...
        state_smt_after
            .insert(giver_bits, giver_account().into())
            .unwrap();
        let token_account_after = Account {
            storage_smt: token_storage_after(),
            ..token_account()
        };
        state_smt_after
            .insert(token_bits, token_account_after.into())
            .unwrap();
...
fn insert_storage(smt: &mut Smt, slot: U256, value: U256) {
    let mut bytes = [0; 32];
    slot.to_big_endian(&mut bytes);
    let key = keccak(bytes);
    let bits = key.into();
    smt.insert(bits, value.into()).unwrap();
}
