...
\begin{enumerate}
  ...
  \item sends the final value for each running product $P_{i, j}^l$ and $P^S_1$ to the verifier,
  \item sends a commitment to $P_i^l$  and $P^S$ to the verifier.
\end{enumerate}
Then, for each STARK $S$, the verifier:
\begin{enumerate}
  \item computes the product $P = \prod_i P_{i, l}^l$,
  \item checks that $P = P^S_1$,
