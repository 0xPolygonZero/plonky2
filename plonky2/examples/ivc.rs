...
fn dummy_proof<F: RichField + Extendable<D>, C: GenericConfig<D, F = F>, const D: usize>(
    ...
    let mut builder = CircuitBuilder::<F, D>::new(config.clone());
     ...
     // The arithmetic circuit for Round function
    let init_target = builder.add_virtual_target();
    let step_target = builder.add_virtual_target();
    let new_value_target = builder.add_virtual_target();
    builder.register_public_input(init_target);
    builder.register_public_input(step_target);
    builder.register_public_input(new_value_target);
    ...
    inputs.set_target(init_target, F::ONE)?;
    inputs.set_target(step_target, F::ZERO)?;
    inputs.set_target(new_value_target, F::ONE)?;
    ...
    InnerC::Hasher: AlgebraicHasher<F>,
{
    ...
    let v0 = inner_proof.public_inputs[0];
    let t = inner_proof.public_inputs[1];
    let vt = inner_proof.public_inputs[2];
    ...
    let i_target = builder.constant(t + F::ONE);
    let mut new_value_target = value_target;
    new_value_target = builder.mul(new_value_target, i_target);
    builder.register_public_input(init_target);
    builder.register_public_input(step_target);
    builder.register_public_input(new_value_target);
    ...
    pw.set_target(init_target, v0)?;
    pw.set_target(step_target, t + F::ONE)?;
    pw.set_target(value_target, vt)?;
    ...
    println!(
        "F^[{}] == {}",
        proof.public_inputs[1], proof.public_inputs[2]
    );
