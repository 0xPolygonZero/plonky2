...
/// Represents a batch FRI oracle, different from `PolynomialBatch` it supports polynomials with difference degrees.
#[derive(Eq, PartialEq, Debug)]
pub struct MultiPolynomialBatch<F: RichField + Extendable<D>, C: GenericConfig<D, F = F>, const D: usize>
{
    pub polynomials: Vec<Vec<PolynomialCoeffs<F>>>,
...
for MultiPolynomialBatch<F, C, D>
{
    fn default() -> Self {
        MultiPolynomialBatch {
            polynomials: Vec::new(),
            field_merkle_tree: FieldMerkleTree::default(),
            degree_logs: Vec::new(),
            rate_bits: 0,
            blinding: false,
        }
...
MultiPolynomialBatch<F, C, D>
{
    /// Creates a list polynomial commitment for the polynomials interpolating the values in `values`.
    pub fn from_values(
        values: Vec<Vec<PolynomialValues<F>>>,
        rate_bits: usize,
        blinding: bool,
        cap_height: usize,
        timing: &mut TimingTree,
        fft_root_table: Option<Vec<&FftRootTable<F>>>,
    ) -> Self {
        let coeffs = timed!(
            timing,
            "IFFT",
            values.iter().map(|value| value.into_par_iter().map(|v| v.ifft()).collect::<Vec<_>>() ).collect()
        );
    ...
    pub fn from_coeffs(
        polynomials: Vec<Vec<PolynomialCoeffs<F>>>,
        rate_bits: usize,
        blinding: bool,
        cap_height: usize,
        timing: &mut TimingTree,
        ...
        let degree_logs = polynomials.iter().map(|p|log2_strict(p[0].len())).collect();
