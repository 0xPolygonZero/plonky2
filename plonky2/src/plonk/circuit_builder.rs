...
use crate::gates::constant::{ConstantGate, ConstantOperation};
...
impl<F: RichField + Extendable<D>, const D: usize> CircuitBuilder<F, D> {
    ...
    pub fn constant(&mut self, c: F) -> Target {
        ...
        self.add_operation(ConstantOperation {
            target,
            constant: c,
            gate: ConstantGate {
                num_consts: self.config.constant_gate_size,
            },
        });
        target
    }
    ...
    pub fn build<C: GenericConfig<D, F = F>>(mut self) -> Circuit<F, D> {
        ...
        // Fill batched gates
        // TODO: Allow combining constants, for example for the ConstantGate.
        // TODO: Handle generators
        let mut gates_to_targets: HashMap<GateRef<F, D>, HashMap<Vec<F>, Vec<Vec<Target>>>> =
            HashMap::new();
        for op in &self.operations {
            if let Some(gate) = op.0.gate() {
                let constant_to_targets = gates_to_targets
                    .entry(gate)
                    .or_insert_with(|| HashMap::new());
                let constants = op.0.constants();
                let targets = constant_to_targets
                    .entry(constants)
                    .or_insert_with(Vec::new);
                gate.0.add_operation(op.0.targets(), targets);
            } else {
