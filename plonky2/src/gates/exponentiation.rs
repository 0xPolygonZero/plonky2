...
#[derive(Clone, Debug)]
pub(crate) struct ExponentiationGate<F: PlonkyField<D>, const D: usize> {
    pub num_power_bits: usize,
    pub _phantom: PhantomData<F>,
}

impl<F: PlonkyField<D>, const D: usize> ExponentiationGate<F, D> {
...
impl<F: PlonkyField<D>, const D: usize> Gate<F, D> for ExponentiationGate<F, D> {
    fn id(&self) -> String {
        format!("{:?}<D={}>", self, D)
    }
...
impl<F: PlonkyField<D>, const D: usize> PackedEvaluableBase<F, D> for ExponentiationGate<F, D> {
...
#[derive(Debug)]
struct ExponentiationGenerator<F: PlonkyField<D>, const D: usize> {
    gate_index: usize,
    gate: ExponentiationGate<F, D>,
}

impl<F: PlonkyField<D>, const D: usize> SimpleGenerator<F> for ExponentiationGenerator<F, D> {
