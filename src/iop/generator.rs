...
pub trait WitnessGenerator<F: Field>: 'static + Send + Sync {
    ...
    fn run(&self, witness: &PartialWitness<F>, out: &mut GeneratedValues<F>) -> bool;
}
...
impl<F: Field> SimpleGenerator<F> for RandomValueGenerator {
    ...
    fn run_once(&self, _witness: &PartialWitness<F>, out: &mut GeneratedValues<F>) {
        ...
        out.set_target(self.target, random_value);
    }
}
...
impl<F: Field> SimpleGenerator<F> for NonzeroTestGenerator {
    ...
    fn run_once(&self, witness: &PartialWitness<F>, out: &mut GeneratedValues<F>) {
        ...
        out.set_target(self.dummy, dummy_value);
    }
}
