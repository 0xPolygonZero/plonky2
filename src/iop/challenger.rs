...
impl RecursiveChallenger {
    pub(crate) fn new<F: Field64 + Extendable<D>, const D: usize>(
    ...
    pub(crate) fn get_challenge<F: Field64 + Extendable<D>, const D: usize>(
        &mut self,
        builder: &mut CircuitBuilder<F, D>,
    ) -> Target {
    ...
    pub(crate) fn get_n_challenges<F: Field64 + Extendable<D>, const D: usize>(
        &mut self,
        builder: &mut CircuitBuilder<F, D>,
        n: usize,
    ) -> Vec<Target> {
    ...
    pub fn get_hash<F: Field64 + Extendable<D>, const D: usize>(
        &mut self,
        builder: &mut CircuitBuilder<F, D>,
    ) -> HashOutTarget {
        HashOutTarget {
    ...
    pub fn get_extension_challenge<F: Field64 + Extendable<D>, const D: usize>(
        &mut self,
        builder: &mut CircuitBuilder<F, D>,
    ) -> ExtensionTarget<D> {
    ...
    /// Absorb any buffered inputs. After calling this, the input buffer will be empty.
    fn absorb_buffered_inputs<F: Field64 + Extendable<D>, const D: usize>(
        &mut self,
        builder: &mut CircuitBuilder<F, D>,
    ) {
        if self.input_buffer.is_empty() {
