...
impl<F: Field64 + Extendable<D>, const D: usize, const B: usize> Gate<F, D> for BaseSumGate<B> {
    fn id(&self) -> String {
        format!("{:?} + Base: {}", self, B)
    }
...
impl<F: Field64, const B: usize> SimpleGenerator<F> for BaseSplitGenerator<B> {
