...
#[derive(Clone, Debug)]
pub(crate) struct ExponentiationGate<F: Field64 + Extendable<D>, const D: usize> {
    pub num_power_bits: usize,
    pub _phantom: PhantomData<F>,
}

impl<F: Field64 + Extendable<D>, const D: usize> ExponentiationGate<F, D> {
...
impl<F: Field64 + Extendable<D>, const D: usize> Gate<F, D> for ExponentiationGate<F, D> {
    fn id(&self) -> String {
        format!("{:?}<D={}>", self, D)
    }
...
#[derive(Debug)]
struct ExponentiationGenerator<F: Field64 + Extendable<D>, const D: usize> {
    gate_index: usize,
