...
/// given point.
#[derive(Clone, Debug)]
pub(crate) struct InterpolationGate<F: Field64 + Extendable<D>, const D: usize> {
    pub num_points: usize,
    _phantom: PhantomData<F>,
}

impl<F: Field64 + Extendable<D>, const D: usize> InterpolationGate<F, D> {
...
impl<F: Field64 + Extendable<D>, const D: usize> Gate<F, D> for InterpolationGate<F, D> {
    fn id(&self) -> String {
        format!("{:?}<D={}>", self, D)
    }
...
#[derive(Debug)]
struct InterpolationGenerator<F: Field64 + Extendable<D>, const D: usize> {
    gate_index: usize,
